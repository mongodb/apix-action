...
pub struct CreatedPullRequest {
    ...
    /// Workflows removed from the target repository.
    #[serde(default)]
    pub removed: Vec<String>,
}

/// Workflow files a target repository gained, changed, or lost.
#[derive(Debug, Default, PartialEq, Eq)]
pub struct ChangedWorkflows {
...
fn changed_workflows(path: &Path) -> Result<ChangedWorkflows> {
    ...
    let mut actions = ChangedWorkflows::default();
    for status in statuses.iter() {
        let path = status.path().context("reading changed path")?;
        let Some(path) = path.strip_prefix(".github/workflows/") else {
            continue;
        };
        // Retired workflows are reported separately so reviewers see what is being removed.
        if status.status().is_wt_deleted() {
            actions.removed.push(path.to_owned());
        } else {
            actions.changed.push(path.to_owned());
        }
    }
    actions.changed.sort();
    actions.changed.dedup();
    actions.removed.sort();
    actions.removed.dedup();
    Ok(actions)
}

// Build the Markdown body listing workflows changed for one repository.
fn description(actions: &ChangedWorkflows) -> String {
    let list = |actions: &[String]| {
        actions
            .iter()
    ...
    let mut body = format!(
        ...
        list(&actions.changed)
    );
    if !actions.removed.is_empty() {
...
mod tests {
    ...
    use super::{
        ChangedWorkflows, CreatedPullRequest, changed_workflows, description, summary_directory,
    };

    #[test]
    fn description_lists_changed_actions() {
        let actions = ChangedWorkflows {
            changed: vec!["dependabot-auto-merge.yaml".to_string()],
            removed: Vec::new(),
        };
        assert_eq!(
            description(&actions),
    ...
    #[test]
    fn description_lists_removed_actions() {
        let actions = ChangedWorkflows {
            changed: Vec::new(),
            removed: vec!["dependabot-auto-merge.yaml".to_string()],
        };
        assert!(
            description(&actions).ends_with("## Actions removed\n\n- `dependabot-auto-merge.yaml`")
        );
    }
    ...
    fn summary_lists_repositories_and_changed_files() -> Result<()> {
        ...
        fs::write(
            ...
            serde_json::to_string(&vec![CreatedPullRequest {
                ...
                removed: vec!["old.yaml".to_string()],
            }])?,
        )?;

        assert_eq!(
            summary_directory(&directory)?,
            "## Created pull requests\n\n- [`mongodb/example`](https://github.com/mongodb/example/pull/1): `ci.yaml`, `old.yaml` (removed)\n"
        );
    ...
    #[test]
    fn changed_workflows_reports_deleted_workflows_separately() -> Result<()> {
        let unique = SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)?
            .as_nanos();
        let root =
            std::env::temp_dir().join(format!("apix-prs-del-{}-{unique}", std::process::id()));
        let workflows = root.join(".github/workflows");
        fs::create_dir_all(&workflows)?;
        fs::write(workflows.join("retired.yaml"), "workflow")?;
        let repository = git2::Repository::init(&root)?;

        // Commit the workflow so its removal registers as a tracked deletion.
        let mut index = repository.index()?;
        ...
        fs::write(workflows.join("added.yaml"), "workflow")?;

        assert_eq!(
            changed_workflows(&root)?,
            ChangedWorkflows {
                changed: vec!["added.yaml".to_string()],
                removed: vec!["retired.yaml".to_string()],
            }
        );
